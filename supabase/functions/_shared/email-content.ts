...
export function paymentStatusLabel(
  registration: Pick<EmailRegistration, 'payment_status' | 'legacy_payment_paid'>,
): string {
  if (registration.payment_status === 'paid' || registration.legacy_payment_paid) {
    return 'Verified';
  }
...
function paymentFacts(event: EmailEvent, registration: EmailRegistration): string {
  ...
  return emailField('Payment method', paymentMethodLabel(registration.payment_method))
...
export function buildConfirmedRegistrationEmail(input: {
  ...
  const fields = formFields.map((item) => {
    ...
    return emailField(item.label, Array.isArray(value) ? value.join(', ') : value);
  }).join('');
  ...
  const html = wrapEmail(`
    ...
    <div class="body">
      ...
      <a href="${escapeHtml(cancelUrl)}" class="cancel-link">Cancel Registration</a>
    </div>
...
export function buildReminderEmail(input: {
  ...
  const html = wrapEmail(`
    ...
    <div class="body">
      ...
      <a href="${escapeHtml(calendarUrl)}" style="color:#2563eb;font-size:13px;text-decoration:underline;" target="_blank">Add to Google Calendar</a>
    </div>
