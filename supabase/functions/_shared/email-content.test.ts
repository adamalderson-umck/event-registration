...
describe('email content', () => {
  ...
  it('maps selected payment methods and current statuses to approved labels', () => {
    ...
    expect(paymentStatusLabel({ payment_status: 'not_required' })).toBe('Not required');
  });
  ...
  it('uses the standard fallback and reads verified payment state in reminders', () => {
    ...
    const reminder = buildReminderEmail({
      ...
      eventDate: 'Saturday, August 15, 2026',
      eventTime: '9:00 AM',
      calendarUrl: 'https://calendar.google.com/calendar/render?action=TEMPLATE&text=Parking',
    });
    expect(reminder.subject).toBe('Reminder: Fall Parking is coming up!');
    expect(reminder.html).toContain('Payment method</div><div class="field-value">Pay in Person');
  ...
  it('rejects missing required custom text for parking confirmations and reminders', () => {
    expect(() => buildConfirmedRegistrationEmail({
      event: { title: 'Parking', event_type: 'parking', confirmation_message: '   ' },
      registration: parkingRegistration,
      formFields: [],
      eventDate: null,
      cancelUrl: 'https://events.example/cancel',
    ...
    expect(() => buildReminderEmail({
      event: { title: 'Dinner', event_type: 'standard', reminder_message: '' },
